...
export class Call extends Instruction {
    ...
    execute(env) {
        ...
        if (func == null)
            throw new error("Semantico", `No enconte la funcion con el nombre '${this.id}'`, this.line, this.column);
        ...
        if (this.expresiones.length != func.parametros.length)
            throw new error("Semantico", `No enconte la funcion con nombre '${this.id}' con todos estos parametros`, this.line, this.column);
        ...
        for (let i = 0; i < func.parametros.length; i++) {
            ...
            if (element == "number" && array[i] == Type.NUMBER ||
                element == "string" && array[i] == Type.STRING ||
                element == "boolean" && array[i] == Type.BOOLEAN) {
                //significa que son del mismo tipo
            }
            else
                throw new error("Semantico", `Error de parametros, no se esperaba un tipo [${get(array[i])}] en la posicion ${i + 1} de los parametros de la funcion`, this.line, this.column);
        }
